...
void AppAR::setup() {
	...
	AppAR::loadCameraCalibration("CAMARA", intrinsicMatrix, distortionParameters);
	markerDictionary = cv::aruco::getPredefinedDictionary(cv::aruco::PREDEFINED_DICTIONARY_NAME::DICT_4X4_50);
	board = cv::aruco::GridBoard::create(5, 7, 0.04, 0.01, markerDictionary);
...
void AppAR::display() {
	...
	mvStack.top() *= glm::scale(glm::mat4(1.0f),
		glm::vec3(1.8f, 1.8f, 1.8f));  // make the sun bigger
	...
	mvStack.top() *= glm::translate(glm::mat4(1.0f),
		glm::vec3(sin((float)time / 2.0f) * 5.0, cos((float)time / 2.0f) * 5.0,
			0.0f));  // planet position
	mvStack.push(mvStack.top());  // duplicating
	mvStack.top() *= glm::rotate(glm::mat4(1.0f), (float)time,
	...
	earth->setMat4("modelview", mvStack.top());
	...
	moon->setMat4("modelview", mvStack.top());
